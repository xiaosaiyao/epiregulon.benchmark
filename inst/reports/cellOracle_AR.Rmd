...
library(ArchR)
proj <- loadArchRProject("/gstore/project/ar_ligands/AR/scRNAseq/NGS4557_BRG1i/OUTPUT/gp.sa.archr/")
proj <- addCellColData(ArchRProj = proj, data  = sapply(strsplit(proj$hash_assignment, split = "_"), "[",2), name = "cell_line", cells = proj$cellNames)
...
barcode_tab <- data.frame(barcode = colnames(GeneExpressionMatrix))
barcode_tab$sample_id <- gsub("(.*)(#.*)", "\\1",barcode_tab$barcode)
barcode_tab$barcode <- gsub(".*#", "",barcode_tab$barcode)
barcode_tab$barcode <- gsub("-", "\\.",barcode_tab$barcode)
barcode_tab$hash_assignment <- GeneExpressionMatrix$hash_assignment 
```
