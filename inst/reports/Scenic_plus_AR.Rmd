...
adata <- get_annotated_data(barcode_tab, data_paths = as.list(params$data_file_paths), sample_names = as.list(sample_names), n_top_genes=as.integer(2000), python_library = "scenicplus", variable = params$group_variable)
```
```{r cistopic cache = TRUE, cache.lazy = FALSE}
...
cistopic_obj <- find_topics(adata, as.list(sample_names), as.list(params$data_file_paths), work_dir, params$temp_dir, as.list(paths_to_matrices), params$n_cpu, params$group_variable, params$save_results, cistopic_file, params$save_path,
                            current_cell_line)
```
```{r scenicplus_main cache = TRUE, cache.lazy = FALSE}
eRegulons <- run_scenicplus(work_dir, adata, cistopic_obj, params$n_cpu, params$group_variable, params$save_results, paste0("scenicplus_AR_", current_cell_line), params$save_path)
