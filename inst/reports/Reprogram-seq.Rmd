...
for(i in seq_along(params$treatments)){
    if(is.null(params$treatments[[i]]))
        control_treatment[i] <- get_complementary_names(names(params$treatments)[i], unique(GeneExpressionMatrix$heash_assignment))
    else
        control_treatment[i] <- paste0(params$treatments[[i]], collapse = "|")
}

group_combinations$control_treatment <- control_treatment
group_combinations$transcription_factor <- unlist(lapply(function(x) map_treatment_to_tf(x, regulon), names(group_combinations$treatments)))
