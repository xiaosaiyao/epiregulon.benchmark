...
cellinfo = data.frame(cells = gsub("reprogram#","",colnames(peakMatrix)))
...
#use_condaenv(params$conda_envir)
#source_python(system.file("base_GRN.py", package = "epiregulon.benchmark"))
#base_GRN <- prepare_base_GRN(all_peaks, conns)
#use_condaenv(condaenv = "/gstore/home/wlodarct/miniforge3/envs/test")
```
...
base_GRN <- get_base_GRN(conns, all_peaks, association_cutoff = 0.5)
barcode_tab <- data.frame(barcode = colnames(mae[["GeneExpressionMatrix"]]))
barcode_tab$sample_id <- gsub("(.*)(#.*)", "\\1",barcode_tab$barcode)
barcode_tab$barcode <- gsub(".*#", "",barcode_tab$barcode)
barcode_tab$barcode <- gsub("-", "\\.",barcode_tab$barcode)
barcode_tab[[params$group_variable]] <- colData(mae[["GeneExpressionMatrix"]])$hash_assignment 
```
...
adata <- get_annotated_data(barcode_tab,
                            ...
                            python_library = "cellOracle",
                            variable = params$group_variable)
res <- calculate_GRN(adata, base_GRN)
if (params$save_results) saveRDS(res, file.path(save_path, "cellOracle_Reprogram.rds"))
links <- res[[1]]
