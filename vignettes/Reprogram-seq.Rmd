...
```
```{r child = get_matrices_reprogram-seq.Rmd, cache = TRUE, cache.lazy = FALSE}
```
```{r file = params$regulon_script_path, eval = FALSE}
```
```{r child = pruned_regulon_reprogram-seq.Rmd, eval = eval_regulon, echo = eval_regulon}
```
...
```{r child = "Pando_reprogram-seq.Rmd", cache = TRUE, eval = params$include_other_packages}
```
...
```{r child = "cellOracle_reprogram-seq.Rmd", cache = TRUE, eval = params$include_other_packages}
```
...
```{r child = "FigR_reprogram-seq.Rmd", cache = TRUE, eval = params$include_other_packages}
```
