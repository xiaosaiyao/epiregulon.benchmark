...
```{r child = "pruned_regulon_reprogram-seq.Rmd", warning = FALSE, results = "hide", cache = TRUE}
```
...
```{r child = "Pando_reprogram-seq.Rmd", cache = TRUE}
```
...
```{r child = "cellOracle_reprogram-seq.Rmd", cache = TRUE}
```
...
```{r child = "FigR_reprogram-seq.Rmd", cache = TRUE}
```
