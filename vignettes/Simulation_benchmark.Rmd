...
for(i in seq_along(score_matrices)){
    method_res <- assessActivityAccuracy(GRN = GRN, 
                                              sim_options = options_, 
                                              basic_sim_res = sim_res, 
                                              activities_obs = score_matrices[[i]])$interchange_number
    for(i in aeq_along(method_res)){
        method_res[[i]] <- data.frame(interchnages = method_res[[i]], tf = names(method_res)[i], method = method)
    }
    
    df <- rbind(df, data.frame(tf = names(method_res$interchanges), method = methods[i], interchanges = method_res$interchanges))
}
...
for(i in seq_along(score_matrices)){
    ...
    df <- rbind(df, do.call("rbind", method_res))
}
boxplot(df$value~df$method, xlab = "Weight method", ylab = "Rankings mismatch")
```
